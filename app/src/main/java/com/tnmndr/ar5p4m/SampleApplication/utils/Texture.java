...
package com.tnmndr.ar5p4m.SampleApplication.utils;
...
public class Texture
{
    ...
    public static Texture loadTextureFromSDCard(String fileName)
   {
       ...
       try
       {
           ...
           File fileToLoad = new File (sdDir + "/Android/data/com.tnmndr.ar5p4m/files/" + "texture.jpg") ;
           ...
           int halfheight = bitMap.getHeight()   ;
           int halfwidth = bitMap.getWidth()  ;

           return loadTextureFromIntBuffer(data, halfwidth ,
                   halfheight );
       }
       catch (Exception e)
       {
           Log.e(LOGTAG, "Failed to log texture-sd '" + fileName + "' from SD");
           Log.i(LOGTAG, "failure" + e.getMessage());
           return null;
       }
   }
  ...
  //    } catch (IOException e)
  //    {
  //        Log.e(LOGTAG, "Failed to log texture-apk '" + fileName + "' from APK");
  ...
  //}
       ...
       int height)
   {
       ...
       for (int p = 0; p < numPixels; ++p)
       {
           ...
           dataBytes[p * 4 + 2] = (byte) (colour >>> 0) ; // B
